...
var unbxdAutoSuggestFunction = function($, Handlebars, undefined) {
  ...
  $.extend(autocomplete.prototype, {
    ...
    wire: function() {
      ...
      $(
        ".unbxd-as-wrapper"
      ).on("mouseover", "ul.unbxd-as-maincontent", function(e) {
      ...
      $(document).bind("click.auto", function(e) {
        if (e.target == self.input) {
          ...
          self.hasFocus = true;
          if (self.previous === self.$input.val()) self.showResults();
    ...
    moveSelect: function(step) {
      ...
      if (this.activeRow < -1) this.activeRow = lis.size() - 1;
    ...
    trackuniversal: function(type, obj) {
      ...
      if (key) {
        ...
        if (window[key])
          window[
            key
          ]("send", "event", "U_Autocomplete", eventAction, eventLabel, value, {
            nonInteraction: 1
          });
      }
    },
    showResults: function() {
      ...
      var pos = this.$input.offset(),
        ...
        iWidth = this.options.mainWidth > 0
          ? this.options.mainWidth
          : this.$input.innerWidth(),
        ...
        fpos = {
          top:
            pos.top + (isNaN(bt) ? 0 : bt) + this.$input.innerHeight() + "px",
          left: pos.left + "px"
        };
      ...
      if (this.options.template == "2column") {
        this.$results
          ...
          .css("width", this.options.sideWidth + "px");
    ...
    getfilteredPopularProducts: function() {
      var self = this;
      var url =
        "http://search.unbxdapi.com/" +
        this.options.APIKey +
        "/" +
        this.options.siteName +
        "/search?q=" +
        encodeURIComponent(this.params.q) +
        "&rows=" +
        this.options.popularProducts.count;
      $.ajax({ url: url, dataType: "jsonp", jsonp: "json.wrf" }).done(function(
        d
      ) {
    ...
    preparefilteredPopularProducts: function() {
      return (
        (this.options.popularProducts.header
          ? '<li class="unbxd-as-header">' +
              this.options.popularProducts.header +
              "</li>"
          : "") +
