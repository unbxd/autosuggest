...
var unbxdAutoSuggestFunction = function ($, Handlebars, params) {
  ...
  var isDesktop = function () {
    return !(typeof this.options.isMobile === "function"
  ...
  window.autoSuggestObj = $.extend(autocomplete.prototype, {
        ...
        $.ajax({
          ...
          .done(function (data) {
            if (
              ...
              var products = data.response.products.splice(
                0,
                that.options.trendingSearches.maxCount
              );
    ...
    wire: function () {
      ...
      $(".unbxd-as-wrapper").on(
        ...
        function (e) {
          if ($.contains(self.$results[0], e.target) && self.options.filtered) {
            ...
            var dataValue = $(p).attr("data-value")
              ? $(p).attr("data-value")
              : "";
            var dataFiltername = $(p).attr("data-filtername")
              ? $(p).attr("data-filtername")
              : "";
            var dataFiltervalue = $(p).attr("data-filtervalue")
              ? $(p).attr("data-filtervalue")
              : "";
            if (
              !p ||
              p.hasClass("unbxd-as-header") ||
              p.hasClass("unbxd-as-popular-product") ||
              p.hasClass("topproducts") ||
            ...
            if (dataValue) {
              var query =
                dataValue +
                (dataFiltername != ""
                  ? ":" + dataFiltername + ":" + dataFiltervalue
                  : "");
              ...
              if (self.options.filtered) {
                ...
                self.compiledPopularProductHeader = cmpldHeader({
                  hoverSuggestion: dataValue,
                });
              }

              var cmpld = "";
              if (
                self.options.popularProducts.viewMore &&
                self.options.popularProducts.viewMore.enabled
              ) {
                ...
                cmpld = Handlebars.compile(
                  self.preparefilteredPopularProducts() +
                    self.options.popularProducts.viewMore.tpl
                );
              } else {
                cmpld = Handlebars.compile(
                  self.preparefilteredPopularProducts()
              ...
              if (
                self.currentTopResults[query] &&
                self.currentTopResults[query].length > 0
              ) {
            ...
            if (
              self.options.popularProducts.view === "grid" &&
              self.options.popularProducts.rowCount
            ) {
              $(".unbxd-as-sidecontent")
                .find("li.unbxd-as-popular-product-grid")
                .css(
                  "width",
                  100 / self.options.popularProducts.rowCount + "%"
                );
            }
          }
        }
        ...
        if (e.target == self.input) {
          ...
          if (self.previous === self.$input.val())
            if (self.$results.find(".unbxd-as-trending").length) {
              ...
              self.$results.find(".unbxd-as-maincontent").length ||
              self.$results.find(".unbxd-as-sidecontent").length
            ) {
          ...
          self.options.popularProducts.viewMore.redirect(
            self.hoveredQuery || self.$input.val()
          );
          ...
          if (
            ...
            $et.hasClass("unbxd-as-popular-product-cart-button") &&
            typeof self.options.onCartClick == "function"
          ) {
            self.log("BUTTON click");
            var data = p.data();
            data.quantity = parseFloat(
              p.find("input.unbxd-popular-product-qty-input").val()
            );
            ...
            self.options.onCartClick.call(
              self,
              data,
              self.currentResults.POPULAR_PRODUCTS[parseInt(data["index"])]
                ._original
            ) && self.hideResults();
      ...
      if (
        self.options.trendingSearches.enabled &&
        self.clickResults.TRENDING_QUERIES.length
      ) {
    ...
    moveSelect: function (step) {
      ...
      if (this.activeRow < -1)
        this.activeRow =
          (typeof lis.size === "function" ? lis.size() : lis.length) - 1;
      else if (this.activeRow == -1) this.$input.focus();
      else if (
        this.activeRow >=
        (typeof lis.size === "function" ? lis.size() : lis.length)
      ) {
        this.activeRow = -1;
        this.$input.focus();
      }
      ...
      if (
        ...
        if (this.options.filtered && this.activeColumn === 0) {
          var dataValue = $(lis[this.activeRow]).attr("data-value")
            ? $(lis[this.activeRow]).attr("data-value")
            : "";
          var dataFiltername = $(lis[this.activeRow]).attr("data-filtername")
            ? $(lis[this.activeRow]).attr("data-filtername")
            : "";
          ...
          if (this.options.filtered) {
            ...
            var cmpldHeader = Handlebars.compile(ppHeader);
            this.compiledPopularProductHeader = cmpldHeader({
              hoverSuggestion: dataValue,
            });
          }
          ...
          if (
            ...
            this.currentTopResults[query].length > 0
          ) {
            $(".unbxd-as-sidecontent").html(
              cmpld({
          ...
          if (
            this.options.popularProducts.view === "grid" &&
            this.options.popularProducts.rowCount
          ) {
            this.$results
              .find("ul li.unbxd-as-popular-product-grid")
              .css("width", 100 / this.options.popularProducts.rowCount + "%");
          }
        }
        ...
        if (this.options.filtered) {
          ...
          if (
            ...
            this.currentTopResults[this.previous].length > 0
          )
            $(".unbxd-as-sidecontent").html(
              cmpld({
    ...
    selectCurrent: function (e) {
      var li = this.$results.find("li." + this.selectedClass),
        self = this;
      if (li.length) {
        ...
        if (
          typeof this.options.onSimpleEnter == "function" &&
          (this.lastKeyPressCode == 10 || this.lastKeyPressCode == 13)
        ) {
    ...
    selectItem: function (data, e) {
      ...
      if (
        typeof this.options.onItemSelect == "function" &&
        ...
        if (data.sorted) {
          this.options.onItemSelect.call(
            this,
            data,
            this.currentResults["SORTED_SUGGESTIONS"][parseInt(data["index"])]
              ._original,
            e
          );
        } else {
          this.options.onItemSelect.call(
            this,
            data,
            this.currentResults[data.type][parseInt(data["index"])]._original,
            e
          );
        }
        ...
        this.options.onItemSelect.call(
          this,
          data,
          this.currentTopResults[data.src][parseInt(data["index"])]._original,
          e
        );
      }
    },
    ...
    getEventLabel: function (autosuggest) {
      ...
      var filter =
        params.field_name && params.field_value
          ? params.field_name + ":" + params.field_value
          : undefined;
      var types = {
    ...
    trackclassical: function (type, obj) {
      var key = this.options.integrations["classical"],
        eventAction = this.getEventAction(
          obj.autosuggestParams.autosuggest_type
        ),
    ...
    showResults: function () {
      ...
      var posSelector = this.options.inputContainerSelector
      ...
      if (this.options.platform == "io") {
        ...
        totalWidth = this.options.preferInputWidthTotalContent
          ? posSelector.outerWidth()
          : this.options.sideContentOn && this.options.sideContentOn === "left"
          ? pos.left + posSelector.outerWidth()
          : document.body.clientWidth - pos.left;
        if (totalWidth > document.body.clientWidth) {
          totalWidth = document.body.clientWidth;
        }
        if (totalWidth > 788 && totalWidth < 2000) {
          totalWidth = this.options.totalWidthPercent
            ? (this.options.totalWidthPercent * totalWidth) / 100
            : (70 * totalWidth) / 100;
        ...
        if (this.options.template == "1column") {
          var preferInputWidthMainContent =
          ...
          if (isDesktop.call(this)) {
            ...
            $(".unbxd-as-popular-product-info").addClass(
              "unbxd-1column-popular-product-desktop"
            );
          }
          mwidth = preferInputWidthMainContent
            ? posSelector.outerWidth()
            : (60 * totalWidth) / 100;
        } else {
          ...
          mwidth = this.options.mainWidthPercent
            ? (this.options.mainWidthPercent * totalWidth) / 100
            : (30 * totalWidth) / 100;
        }
      }
      ...
      var iWidth =
          this.options.mainWidth > 0
            ? this.options.mainWidth
            : totalWidth
            ? mwidth
            : posSelector.innerWidth(),
        ...
        fpos = {
          top:
            pos.top +
            (isNaN(bt) ? 0 : bt + bb) +
            posSelector.innerHeight() +
            "px",
          left: pos.left + "px",
        };

      var trendingWidth = this.options.trendingSearches.preferInputWidthTrending
        ? posSelector.outerWidth()
        : fwidth;
      ...
      if (this.options.template == "2column") {
        ...
        var swidth =
          this.options.sideWidth !== this.default_options.sideWidth
        ...
        if (
          this.$results.find("ul.unbxd-as-sidecontent").length > 0 &&
          this.options.sideContentOn == "left"
        ) {
          var lwidth =
            pos.left + posSelector.outerWidth() > document.body.clientWidth
    ...
    onChange: function () {
      ...
      if (
        this.lastKeyPressCode == 46 ||
        (this.lastKeyPressCode > 8 && this.lastKeyPressCode < 32)
      ) {
        if (
          this.lastKeyPressCode == 27 &&
          typeof this.input.lastSelected == "object"
        ) {
        ...
        if (v.length >= this.options.minChars) {
          ...
          if (
            !(
              ...
              this.clickResults.TRENDING_QUERIES.length > 0 &&
              v === ""
            )
          ) {
            this.$results.hide();
    ...
    autosuggestUrl: function () {
      ...
      if (
        ...
        typeof this.options.customQueryParse === "function"
      ) {
      ...
      if (this.options.popularProducts.fields.length > 0) {
        var popularProductFields =
          this.options.popularProducts.fields.join(",");
            ...
            if (this.params.filters[x].hasOwnProperty(y)) {
              a.push(
                (
                  x +
                  ':"' +
                  encodeURIComponent(y.replace(/(^")|("$)/g, "")) +
                  '"'
                ).replace(/\"{2,}/g, '"')
              );
            }
    ...
    receiveData: function (data) {
      if (data) {
        ...
        if (
          (!this.hasFocus && (params ? !params.selfServe : true)) ||
          data.response.numberOfProducts == 0 ||
          "error" in data
        ) {
    ...
    isUnique: function (autosuggest, arr) {
      try {
        ...
        for (var k = 0; k < arr.length; k++) {
          ...
          if (
            Math.abs(suggestion.length - autosuggest.length) < 3 &&
            (suggestion.indexOf(autosuggest) != -1 ||
              autosuggest.indexOf(suggestion) != -1)
          ) {
            unique = false;
            break;
          }
    ...
    getfilteredPopularProducts: function () {
      ...
      var self = this,
        urlPath =
          ...
          "/search",
        defaultSearchParams =
          "indent=off&facet=off&analytics=false&redirect=false",
        url =
          urlPath +
          "?q=" +
      ...
      if (self.options.popularProducts.fields.length > 0) {
        var popularProductFields =
          "&fields=" + self.options.popularProducts.fields.join(",");
        ...
        if (i != "POPULAR_PRODUCTS" && this.currentResults.hasOwnProperty(i)) {
          for (var j in this.currentResults[i]) {
            if (this.currentResults[i].hasOwnProperty(j)) {
              ...
              $.ajax(params).done(function (d) {
                var query =
                  d.searchMetaData.queryParams.q +
    ...
    processfilteredPopularProducts: function (query, d) {
      this.currentTopResults[query] = [];
      if (
        d.hasOwnProperty("response") &&
        d.response.hasOwnProperty("products") &&
        ...
        for (var k = 0; k < d.response.products.length; k++) {
          ...
          if (this.options.popularProducts.price) {
            if (
              ...
              o.price =
                this.options.popularProducts.priceFunctionOrKey in doc
                  ? doc[this.options.popularProducts.priceFunctionOrKey]
                  : null;
            } else {
          ...
          if (
            ...
            o.salePrice =
              this.options.popularProducts.salePriceKey in doc
                ? doc[this.options.popularProducts.salePriceKey]
                : null;
          }
          ...
          if (this.options.popularProducts.image) {
            if (
              ...
              o.image =
                this.options.popularProducts.imageUrlOrFunction in doc
                  ? doc[this.options.popularProducts.imageUrlOrFunction]
                  : null;
            }
          }
    ...
    getPopularProductFields: function () {
      ...
      if (
        ...
        typeof this.options.popularProducts.priceFunctionOrKey == "string" &&
        this.options.popularProducts.priceFunctionOrKey
      ) {
        popularProductsFields.push(
          this.options.popularProducts.priceFunctionOrKey
        );
      }
      if (this.options.popularProducts.image) {
        if (
          typeof this.options.popularProducts.imageUrlOrFunction == "string" &&
          this.options.popularProducts.imageUrlOrFunction
        ) {
          popularProductsFields.push(
            this.options.popularProducts.imageUrlOrFunction
      ...
      if (this.options.popularProducts.fields.length > 0) {
        this.options.popularProducts.fields = popularProductsFields.concat(
          this.options.popularProducts.fields
        );
      } else {
    ...
    processPopularProducts: function (doc) {
      ...
      if (this.options.popularProducts.name) {
        o.autosuggest = doc[this.options.popularProducts.nameFunctionOrKey]
          ? doc[this.options.popularProducts.nameFunctionOrKey]
          : doc[this.options.popularProducts.title]
          ? doc[this.options.popularProducts.title]
          : "";
      } else {
      ...
      if (this.options.popularProducts.price) {
        if (
          typeof this.options.popularProducts.priceFunctionOrKey == "function"
        ) {
          o.price = this.options.popularProducts.priceFunctionOrKey(doc);
        } else if (
          typeof this.options.popularProducts.priceFunctionOrKey == "string" &&
      ...
      if (this.options.popularProducts.image) {
        if (
          typeof this.options.popularProducts.imageUrlOrFunction == "function"
        ) {
          o.image = this.options.popularProducts.imageUrlOrFunction(doc);
        } else if (
          typeof this.options.popularProducts.imageUrlOrFunction == "string" &&
      ...
      if (this.options.inFields.showDefault) {
        ...
        Object.keys(doc).forEach(function (item) {
          if (item.length >= 3 && item.substring(item.length - 3) === "_in") {
            ...
            ins[a] = doc[a + "_in"].slice(
              0,
              parseInt(that.options.inFields.noOfInfields)
            );
          }
        });
      } else {
        for (var a in this.options.inFields.fields) {
          if (
            ...
            ins[a] = doc[a + "_in"].slice(
              0,
              parseInt(this.options.inFields.fields[a])
            );
          ...
          for (var i = 0; i < sortedInfields.length; i++) {
            if (sortedInfields[i].filterValue !== "") {
              this.currentResults.IN_FIELD.push({
                ...
                highlighted:
                  this.options.inFields.type === "separate"
                    ? that.prepareinFieldsKeyword(sortedInfields[i].filterValue)
                    : that.highlightStr(doc.autosuggest) +
                      " in " +
                      that.prepareinFieldsKeyword(
                        sortedInfields[i].filterValue
                      ),
                type: doc.doctype,
            ...
            for (var b = 0; b < ins[a].length; b++) {
              if (ins[a][b] !== "") {
                this.currentResults.IN_FIELD.push({
                  ...
                  highlighted:
                    this.options.inFields.type === "separate"
                      ? that.prepareinFieldsKeyword(ins[a][b])
                      : that.highlightStr(doc.autosuggest) +
                        " in " +
                        that.prepareinFieldsKeyword(ins[a][b]),
                  type: doc.doctype,
    ...
    processData: function (data) {
      ...
      for (var x = 0; x < data.response.products.length; x++) {
        ...
        if (this.options.maxSuggestions) {
          if (
            "TOP_SEARCH_QUERIES" == doc.doctype &&
            count["topquery"] > this.currentResults.TOP_SEARCH_QUERIES.length &&
            this.isUnique(doc.autosuggest, uniqueSuggestions)
          ) {
            this.processTopSearchQuery(doc);
          } else if (
            "IN_FIELD" == doc.doctype &&
            count["infields"] + count["key_rem"] + count["top_rem"] >
              infieldsCount &&
            ...
            "KEYWORD_SUGGESTION" == doc.doctype &&
            count["keyword"] > this.currentResults.KEYWORD_SUGGESTION.length &&
            this.isUnique(doc.autosuggest, uniqueInfields)
          ) {
            this.processKeywordSuggestion(doc);
          } else if (
            "POPULAR_PRODUCTS" == doc.doctype &&
            this.options.popularProducts.count >
              this.currentResults.POPULAR_PRODUCTS.length
          ) {
            this.processPopularProducts(doc);
          } else if (
            "PROMOTED_SUGGESTION" == doc.doctype &&
            count["promoted"] >
          ...
          if (
            ...
            "IN_FIELD" == doc.doctype &&
            this.options.inFields.count > infieldsCount &&
      ...
      if (q.indexOf(" ")) {
        ...
        for (var k in arr) {
          ...
          output =
            ...
            output
              .substring(l)
              .replace(new RegExp(this.escapeStr(arr[k]), "gi"), function ($1) {
    ...
    prepareinFieldsKeyword: function (str) {
      if (
        ...
        typeof this.options.customInfieldsFilter === "function"
      ) {
    ...
    prepareinFieldsHTML: function () {
      if (this.options.inFields.type === "inline") {
        ...
        return (
          ...
          "</li>" +
          "{{else}}" +
          '<li class="unbxd-as-insuggestion" style="color:' +
          this.options.theme +
          ';" data-index="{{@index}}" data-type="{{type}}" data-value="{{autosuggest}}" data-filtername="{{filtername}}" data-filtervalue="{{filtervalue}}"  data-source="{{source}}">' +
          "in " +
          (this.options.inFields.tpl
            ? this.options.inFields.tpl
            : this.default_options.inFields.tpl) +
    ...
    preparefilteredPopularProducts: function () {
      return (
        (this.compiledPopularProductHeader
          ...
          : "") +
        "{{#data}}" +
        '<li class="unbxd-as-popular-product ' +
        (this.options.popularProducts.view === "grid"
    ...
    prepareHTML: function () {
      ...
      if (this.options.template === "1column") {
        if (
          ...
          (self.currentResults["IN_FIELD"].length ||
            self.currentResults["KEYWORD_SUGGESTION"].length ||
      ...
      if (
        !self.currentResults["IN_FIELD"].length &&
        !self.currentResults["KEYWORD_SUGGESTION"].length &&
        !self.currentResults["POPULAR_PRODUCTS"].length &&
        !self.currentResults["TOP_SEARCH_QUERIES"].length &&
        !self.currentResults["PROMOTED_SUGGESTION"].length &&
        ...
        if (typeof this.options.noResultTpl === "function") {
          html =
            ...
            this.options.noResultTpl.call(
              self,
              encodeURIComponent(self.params.q)
            ) +
            "</li></ul>";
      ...
      if (this.options.template === "2column") {
        ...
        if (mainlen == 0 && sidelen != 0) {
          ...
          this.options.sideTpl.forEach(function (key) {
            if (
              self.options.sortByLength &&
              (key == "topQueries" ||
                key == "keywordSuggestions" ||
                key == "promotedSuggestions")
            ) {
              return;
            }
      ...
      if (!noResults && mainlen > 0) {
        ...
        if (this.options.sortByLength) {
          if (sidelen === 0) {
            mainHtml =
              ...
              self["prepareSortedSuggestionsHTML"]();
          } else {
        ...
        this.options.mainTpl.forEach(function (key) {
          if (
            self.currentResults[self.standardizeKeys(key)].length &&
            topQuery === ""
          ) {
            topQuery =
              self.currentResults[self.standardizeKeys(key)][0]["autosuggest"];
          }
